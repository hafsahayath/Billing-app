...
const TopProductsSold = ({bills, products}) => {
    ...
    lineItemProducts.forEach(ele=>{
        if(getCountOfPurchaseNumber[findProduct(ele.product, products).name]){
            getCountOfPurchaseNumber[findProduct(ele.product, products).name]=getCountOfPurchaseNumber[findProduct(ele.product, products).name]+ele.quantity
        } else {
            getCountOfPurchaseNumber[findProduct(ele.product, products).name] = ele.quantity
        }
    })

    // const arrToSortData = []
    // for(const key in getCountOfPurchaseNumber){
    //     arrToSortData.push([key, getCountOfPurchaseNumber[key]])
    // }

    // const sortedData = arrToSortData.sort((a,b)=>{
    //     return b[1] - a[1]
    // })
